# AIRPORT MANAGEMENT SYSTEM

## Date: 4/16/2026

### By: FATEMA ALNAJJAS & Hussain Alhakeem & Ali Hasan

#### Links:
[Trello](https://trello.com/b/2zMIHZ1E/%D9%90airport-management) |  [Backend repo](https://github.com/h-alhakeem12/Airport-management-back-end?tab=readme-ov-file)

[GitHub - Fatema](https://github.com/Fatii78) | [LinkedIn - Fatema](https://www.linkedin.com/in/fatema-alnajjas-a57198228)

[GitHub - Hussain](https://github.com/h-alhakeem12) | [LinkedIn - Hussain](https://www.linkedin.com/in/hussain-alhakeem-77454939a?utm_source=share_via&utm_content=profile&utm_medium=member_ios)

[GitHub - Ali](https:) | [LinkedIn - Ali](https:)


***

## ***Description***

#### The Airport Management System is a full-stack application designed to streamline airport operations. The platform provides a dual-interface experience: an Admin Dashboard for managing personnel, flights, and task assignments, and a Staff Portal for employees to track their assigned flights and duties in real-time.

#### By centralizing flight schedules and task management, the system ensures efficient communication between airport administration and ground staff, improving overall operational workflow.

***

## ***Technologies Used***

* Frontend Framework: React.js
* Routing: React Router Dom
* API Client: Axios
* State Management: React Hooks (useState, useEffect)
* Security: JWT-based Authentication
* Styling: CSS

***

## ***Getting Started***

#### 1.  Admin Login: Create and manage staff accounts, flights, and tasks.
#### 2.  Staff Login: View personalized flights and tasks.
#### The project was deployed and can be viewed here:
[Live Demo]()

***

## ***Screenshots***

#### Component Hierarchy
![Component Hierarchy](./images/Component%20.jpeg)
***

## ***Future Updates***

- [ ] Interactive terminal map with gate status indicators.
- [ ] Real-time flight status updates using a third-party aviation API.
- [ ] In-app messaging system for communication between Admins and Crew.

***

## ***Team Contributions***

###  Fatema Alnajjas
-


###  Hussain Alhakeem
-

###  Ali Hassan
-
***

## ***Credits***

Markdown Guide: https://ia.net/writer/support/general/markdown-guide

Markdown Cheatsheet: https://guides.github.com/pdfs/markdown-cheatsheet-online.pdf
